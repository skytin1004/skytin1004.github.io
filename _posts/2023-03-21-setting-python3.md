...
기존에 사용되던 명령어가 새로운 버전에서 작동하지 않을 수도 있습니다. 뿐만 아니라 파이썬 모듈도 특정 버전에서만 지원되는 경우가
있기 때문에 프로젝트 별로 가상환경을 만들어서 파이썬 버전을 관리하는 것이 좋습니다.<br>
이번 포스트에서는 파이썬 가상환경을 만들어서 파이썬을 설치하는 방법을 알아보겠습니다.
## 1. 프로젝트 관리 폴더 생성
...
<span style="color:gray">venv를 이용한 가상환경 설정은 4월 중으로 올리겠습니다.</span>

### 1. 파이썬 프로젝트 폴더 만들기
터미널에서 cd proj 입력합니다.(proj폴더로 들어가는 명령어)


## 3. virtualenv 가상환경 설정

virtual env는 파이썬 비표준 가상환경 라이브러리에 속해있습니다. venv와는 다르게 파이썬 설치 후 virtual env를 따로 설치해야 합니다.<br>
...
### 4. "pythonProject"폴더 안에 가상환경 만들기
...
<span style="color:gray">(리눅스,맥은 `Python3 -m virtualenv env` 를 입력합니다.)</span>
virtualenv <span style="color:#9370DB">env</span>  에서 <span style="color:#9370DB">env</span> 는 가상환경폴더의 이름이며
변경할 수 있습니다.<br>
...
맨 위에 'Location'은 방금 전애 만들었던 'pythonProject'의 파일 경로로 설정합니다.<br>
...
<span style="color:#9370DB">'Interpreter: <No interpreter>'</span> 이렇게 되어있는데 이 줄 맨 오른쪽  <span style="color:#9370DB">`...` </span> 네모 박스 클릭합니다. <br>
