...
데이터 수집을 담당하는 apache-airflow의 원리과 구조를 공부해보고 싶어서 airflow에 contribution을 하게 되었습니다. 
처음 오픈소스에 기여하기 때문에 주요 내용을 수정하는 것이 아니라 airflow의 문서에서 오탈자를 찾아 고치는 방식으로 진행했습니다. 
오픈소스에 기여하는 방법은 정말 다양합니다. 꼭 오픈소스의 핵심 기능을 추가하거나 주요 버그를 수정 하는 것 뿐만 아니라 문서를 변역하거나 오탈자를 수정함으로써 오픈소스에 기여할 수도 있습니다. 
어느정도 이런 방식으로 오픈소스에 기여하면서 파일의 전반적인 구조 및 airflow 내부의 작동원리를 이해하고 파이썬 코드를 수정해서 시스템 내부를 손 보거나 버그를 고치고 새로운 기능을 추가해
보는 것을 목표로 하고 있습니다.
...
저 같은 경우는 airflow 문서의 오탈자를 잡아내거나 주석에서 문법적으로 잘못된 부분을 찾아 Pull request 를 해서 apache-airflow에
컨트리부트 하게 되었습니다.
...
여기서 파일을 수정해 apache의 airflow 레포지토리에 Pull request를 올리면 됩니다.

![img](https://drive.google.com/uc?id=1-iLsCTOpKgfrSHaKPwzyTEicQVx1lc-x)

이렇게 문서나, 코드의 주석에서 문법적인 오류나 오탈자를 수정해 apache/airflow 에 Pull request를 했습니다. 

### 2. Pull request

![img](https://drive.google.com/uc?id=15DYFz10kLovdP5fq0K8AR70jyDv4ez89)

파일 수정을 해서 본인의 레포지토리에 커밋을 했다면 이제 Pull request를 해야 합니다. Contribute → Open Pull Request 클릭합니다.

![img](https://drive.google.com/uc?id=11KYJyqllkdJDDrh1L2Kn2gR7xquc9wvO)

Pull request를 할 때는 제목과 내용을 영어로 상세하게 적어줍니다. 

저는 오탈자 수정 위주의 커밋을 했기 때문에 "Fix Typos and Polish ~" 라는 제목으로 Pull request 했습니다.
...
컨트리뷰션이 완료되면 메인 브렌치에 Pull request한 내용이 나옵니다.
