...
위 사이트에서 마음에 드는 테마를 하나 선택합니다.

> 저는 http://jekyllthemes.org/ 에서 Yet Another Theme 로 선택했습니다.
...
터미널에서 `cd username.github.io` 를 입력합니다. (저는 `cd sms7950.github.io` 로 입력하겠습니다.)
...
구름IDE 서버 실행 코드 : `bundle exec jekyll serve --host 0.0.0.0`

(앞으로 자주 쓰게 될 명령어이니 기억해두시기 바랍니다.)

host를 0.0.0.0으로 설정하는 이유는 구름IDE 에서는 local 호스트에서 서버 실행이 안되기 때문입니다.

'프로젝트' → '실행 URL과 포트'에서 포트번호 4000번으로 설정헀던 주소를 복사 후 주소창에 붙여넣어 웹 페이지가 잘 만들어졌는지 확인합니다.

### Github에 올리기

터미널에서 `git add .` 입력
터미널에서 `git commit -m"first commit"` 입력
터미널에서 `git push` 입력

![img](https://drive.google.com/uc?id=1mPEwSalRKJs9SNaj2m0J0GMPQVpY7wW0)

입력하는 도중 이런 오류가 나올 수도 있는데 이러한 경우 아래 사진 처럼 터미널에 입력해주면 됩니다.

![img](https://drive.google.com/uc?id=1cTCZBuJrxbSLhM7yCW_oJVFH13AIKFbN)

다시 터미널에서 `git commit -m"first commit"` 입력 후
터미널에서 `git push` 입력 
...
※git push가 완료되면 주소창에 username.github.io로 접속해서 깃허브 블로그가 완성이 되었는지 확인합니다.

## 3.jekyll 테마 수정

구름IDE 의 username.github.io 폴더 → config.yml 파일로 이동합니다.  
